...
vec3 GetNightNebula(vec3 viewPos, float VdotU, float VdotS) {
			...
			// only do the wrap around when atmosphere is faded out
			float horizonHideFactor = mix(max0(VdotU), max0(abs(VdotU)), atmFadeoutFactor);
		#else
			float horizonHideFactor = max0(abs(VdotU));
		#endif
	#else
		float horizonHideFactor = max0(VdotU);
	#endif
	#ifdef NEBULA_AT_DAY
		...
		#endif
	#endif
    float nebulaFactor = pow2(horizonHideFactor * nightHideFactor) * invRainFactor;
